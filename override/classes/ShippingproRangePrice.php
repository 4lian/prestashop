...
class ShippingproRangePrice extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'shippingpro_range_price',
		'primary' => 'id_shippingpro_range_price',
		'fields' => array(
	...
	protected $webserviceParameters = array(
			...
			'fields' => array(
				'id_shippingpro_carrier' => array('xlink_resource' => 'carriers'),
			)
	);
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$carrier = new ShippingproCarrier((int)$this->id_shippingpro_carrier);
		$price_list = array();
		foreach ($carrier->getZones() as $zone)
			$price_list[] = array(
				'id_shippingpro_range_price' => (int)$this->id,
				'id_range_weight' => 0,
	...
	*/
	public static function getRanges($id_carrier)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT * 
			FROM `'._DB_PREFIX_.'shippingpro_range_price` 
	...
	public static function isOverlapping($id_carrier, $delimiter1, $delimiter2, $id_rang = null)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
			...
			'.(!is_null($id_rang) ? ' AND `id_shippingpro_range_price` != '.(int)$id_rang : ''));
	}
}