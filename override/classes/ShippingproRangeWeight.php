...
class ShippingproRangeWeight extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'shippingpro_range_weight',
		'primary' => 'id_shippingpro_range_weight',
		'fields' => array(
			'id_shippingpro_carrier' => array('type' => self::TYPE_INT, 'validate' => 'isInt', 'required' => true),
	...
	protected $webserviceParameters = array(
			...
			'fields' => array(
			'id_shippingpro_carrier' => array('xlink_resource' => 'shippingpro_carriers'),
		)
	);
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$shippingpro_carrier = new ShippingproCarrier((int)$this->id_shippingpro_carrier);
		$price_list = array();
		foreach ($shippingpro_carrier->getZones() as $zone)
			$price_list[] = array(
				'id_shippingpro_range_price' => 0,
				'id_shippingpro_range_weight' => (int)$this->id,
				'id_shippingpro_carrier' => (int)$this->id_shippingpro_carrier,
				'id_zone' => (int)$zone['id_zone'],
				'price' => 0,
			);
		$shippingpro_carrier->addDeliveryPrice($price_list);
	...
	*/
	public static function getRanges($id_carrier)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT * 
			FROM `'._DB_PREFIX_.'shippingpro_range_weight` 
			WHERE `id_shippingpro_carrier` = '.(int)$id_carrier.'
	...
	public static function isOverlapping($id_carrier, $delimiter1, $delimiter2, $id_rang = null)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
			...
			'.(!is_null($id_rang) ? ' AND `id_shippingpro_range_weight` != '.(int)$id_rang : ''));
	}
}
